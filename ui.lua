...
local folder
local function createSound(soundId, name, addPreps)
    if typeof(addPreps) ~= 'table' then
        addPreps = {
            Parent = game:service('ReplicatedStorage'),
            Name = 'cap_audio',
        }
    else
        if addPreps['Parent'] == nil then
            addPreps['Parent'] = game:service('ReplicatedStorage')
        end

        if addPreps['Name'] ~= 'cap_audio' then
            addPreps['Name'] = 'cap_audio'
        end
    end

    if not game:service('ReplicatedStorage'):FindFirstChild('cap_audio') then
        local new = instance('Folder', {
            Parent = game:service('ReplicatedStorage'),
            Name = 'cap_audio',
        })
...
local sounds = {
    launch = createSound('11880415383', 'launch'),
    toggleOn = createSound('11880421813', 'toggle_on'),
    toggleOff = createSound('11880421156', 'toggle_off'),
    click = createSound('11880412544', 'click'),
    swoosh = createSound('11880416450', 'swoosh'),
    tabOpen = createSound('11880420098', 'tab_open'),
    tabClose = createSound('11880418818', 'tab_close')
}
...
local sound = {
    ...
    toggle = function(s)
        playSound(s and 'toggleOn' or 'toggleOff')
    end,
    click = function()
        playSound('click')
    end,
    ...
    tab = function(s)
        playSound(s and 'tabOpen' or 'tabClose')
    end
}
    ...
    colorpicker = function(v2, data, obj)
        ...
        spawn(function()
            while true do
                task.wait()
                if rainbow then
                    local savedSpeed = getgenv().rainbowSpeed
                    for a=0,1,0.001* getgenv().rainbowSpeed do
                        wait()
                        if not rainbow then
                            break
                        end
                        if getgenv().rainbowSpeed ~= savedSpeed then
                            break
                        end
                        H = a
                        spawn(function()
...
local updateTable = {} --update table for updating dropdowns and textlabels
new{
    theme = {
    ...
    tabs = {
        {name = 'tab 1', content = {
            {type = 'folder', name = 'folder 1', size = 200, content = {
                {type = 'button', text = 'asd', callback = function()
                    print('hello world')
                end},
                {type = 'slider', text = 'slider', min = 0, max = 75, float = 1, value = 10, deg = ' dicks in your mouth', callback = function(v)
                    print(v)
                end},
                {type = 'colorpicker', text = 'color', color = Color3.new(1, 1, 1), callback = function(c)
                    print(c)
                end}
            }},


            {type = 'button', text = 'asd', tooltip = 'hello my name is john', callback = function()
                print('hello')
            end},
            {type = 'toggle', state = false, text = 'toggle on', callback = function(s)
                print(s)
            end},
            {type = 'keybind', key = 'E', text = 'keybind', callback = function(s)
                print('keybind!')
            end, newCallback = function(s)
                print('key changed!:',s)
            end},
            {type = 'togglebind', state = false, key = 'KeypadNine', text = 'toggle bind', callback = function(s)
                print(s)
            end, newCallback = function(s)
                print('key changed!:',s)
            end},
            {type = 'databox', options = {
                {'option 1', false},
                {'option 2', true},
                {'option 3', false},
                {'hello', true},
                ...
                {'oufkjdhk', true},
                {'nig', false}
            }, text = 'databox', callback = function(s)
                print('new options:')
                for a,v in next, s do
                    print('-'.. a ..' = '.. tostring(v))
                end
            end},
            {type = 'slider', min = 70, max = 100, value = 75, float = 5, deg = 'Â°', text = 'slider', callback = function(v)
                print(v)
            end},
            {type = 'textbox', placeholder = 'input text here', text = 'textbox', clear = false, callback = function(v)
                print(v)
            end},
            {type = 'textbox', placeholder = 'input text here', text = 'textbox (text reset)', callback = function(v)
                print(v)
            end},
            {type = 'colorpicker', color = {255, 255, 255}, text = 'colorpicker', callback = function(c)
                print(c)
            end},
            {type = 'textlabel', text = 'left, {1, 1, 1}, 1', align = 'left', color = Color3.new(1, 1, 1), thickness = 1},
            {type = 'dropdown', text = 'dropdown', options = {
                'option 1',
                'option 2',
                'option 3'
            }, def = 'option 2', callback = function(v)
                print(v)
            end}
        }},
        {name = 'tab 2', size = 3, content = {
            {type = 'button', text = 'button name 1', callback = function()
                print('hello')
            end, tooltip = 'this is a test of tooltips'},
            {type = 'keybind', text = 'this keybind has no tooltip', name = 'asd'},
            {type = 'button', text = 'button name 2', tooltip = 'tooltip test 2'}
        }},
        {name = 'very long tab', text = 'asd', content = {
            {type = 'button', callback = function()
                print('hello')
            end}
        }},
        {name = 'tab 6', text = 'asd', content = {
            {type = 'button', text = 'update textlabel', callback = function()
                updateTable['long name asdshit']({
                    text = 'updated textlabel',
                    color = Color3.new(0.5, 1, 1),
                    align = 'Left',
                    thickness = 3
                })
            end},
            {type = 'textlabel', text = 'long name asdshit', updTable = updateTable},
            {type = 'dropdown', text = 'dropdown', options = {'hello', 'hello2', 'hello3'}, updTable = updateTable},
            {type = 'toggle', text = 'toggle "nigger" option in dropdown', callback = function(s)
                if s then
                    updateTable['dropdown']({'hello', 'hello2', 'hello3', 'nigger'})
                else
                    updateTable['dropdown']({'hello', 'hello2', 'hello3'})
                end
            end},
            {type = 'locked', text = 'Premium only feature'}
        }},
    }
}
